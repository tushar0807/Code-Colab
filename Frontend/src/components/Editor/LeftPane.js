...
function LeftPane({ clients, data, pid }) {
  ...
  const handleCast = (e) => {
    ...
    console.log(socketRef.current, "emitted broadcast", {
      text: castRef.current.value,
      title: `Message from ${localStorage.getItem("uid")}`,
    });
    setCast({
      text: msg,
      title: `Message from ${localStorage.getItem("uid")}`,
  ...
  return (
    ...
    >
      <Center
        sx={{ border: "1px solid green", borderRadius: "4px", maxWidth: "60%" }}
        color="green"
        p="sm"
        m="sm"
        variant="outline"
      >
        <Text mr={"md"} color="green">
          Online
        </Text>
        <RiRadioButtonLine ml="md" color="green" />
      </Center>

      <SimpleGrid cols={2} pl={"16px"} sx={{ marginTop: "20px" }}>
        {clients.map((ele) => (
          <Stack key={ele.username} spacing={0}>
            <Avatar
              name={ele.username}
              p={0}
              ml={"8px"}
              ...
              size={32}
            />
            <Text size="sm" m={0} p={0} color="white">
              {ele.username.split(" ")[0] > 10
                ? ele.username.slice(0, 11)
      ...
      {data?.createdBy === localStorage.getItem("uid") && (
        <div style={{ marginTop: "48px" }}>
          <Input
            ...
            ref={inpRef}
            sx={{ fontWeight: 600, fontSize: "24px", maxWidth: "200px " }}
          />
          <Button
            sx={{
      ...
      {data && (
        <div style={{ marginTop: "24px" }}>
          <Textarea
            p="xs"
            autosize
            placeholder="Broadcast instant message to all active participants"
            ref={castRef}
            sx={{ fontWeight: 600, fontSize: "24px", maxWidth: "200px " }}
          />
          <Button
